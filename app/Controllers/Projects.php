...
class Projects extends Security_Controller {
    ...
    private function can_comment_on_tasks() {
        if ($this->login_user->user_type == "staff"||$this->login_user->user_type == "sponsor") {
            if ($this->can_manage_all_projects()||$this->login_user->user_type == "sponsor") {
                return true;
    ...
    function all_projects($status = "") {
        ...
        if ($this->login_user->user_type === "staff") {
            $view_data["can_edit_projects"] = true;
            $view_data["can_delete_projects"] = true;
...
$builder = $db->table('users');
   $result = $builder->getWhere(array('id' => $this->login_user->id, 'deleted' => 0));
...
//
   ...
   if ($user_info->is_sponsor == "sponsor"&&$data->status=="open") {
            $optoins .=  anchor(get_uri("projects/view/" . $data->id), "Sponsor Project");
        }
    ...
    function view($project_id = 0, $tab = "") {
        ...
        $view_data["show_contract_info"] = true;
        ...
        $view_data["show_timesheet_info"] = true;

        $view_data["show_tasks"] = true;

        $view_data["show_gantt_info"] = true;
